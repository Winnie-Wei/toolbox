...
var Validate = function(){
    var validate = {
         ...
         *  param: value 需要验证的数据
         */
        validEmpty: function(value, msgObj) {
            if (!value) {
                return false;
            }
            value = $.trim(value);
         ...
         */
        validEqual: function(value1, value2) {
         ...
         *  param: type 数字类型
         */
        validNumber: function(text, type) {
            var re = null;
            if (type == 'integer') {
                re = /^[1-9]+[0-9]*]*$/;
                if (!re.test(text)) {
         ...
         *  验证目录
         */
        validPath: function(str) {
            var regx=/^[a-zA-Z]{1}:\\[\w|\\]*$/;
            var h=str.search(/^\/\w*/);
            if ((h==-1) && (!regx.test(str))){
                return false;
            } else {
                return true;
            }
